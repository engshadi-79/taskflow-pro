...
// (Phase 11). due_date in Postgres is a plain `date` - no time zone
// component at all - so "Aug 15" must always mean the same calendar day
// regardless of the viewer's browser offset. `new Date("2026-08-15")`
// parses as UTC midnight, and reading it back with local getters
// (getDate/getMonth) can silently shift a day in negative-UTC time zones.
// Everything here works in UTC-anchored Date objects exclusively (Date.UTC
// to construct, getUTC* to read) so that trap never has a chance to occur.
...
export function weekdayLabel(key: DateKey): string {
  return WEEKDAY_LABEL[parseDateKey(key).getUTCDay()];
}
