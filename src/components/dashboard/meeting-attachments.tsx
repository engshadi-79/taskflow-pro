...
export function MeetingAttachments({
  ...
  const [state, formAction, pending] = useActionState(uploadMeetingAttachment, initialState);
        ...
        {attachments.map((a) => (
          ...
          >
            ...
            {(canDeleteAny || a.uploaded_by === currentUserId) && (
              <DeleteButton id={a.id} meetingId={meetingId} filePath={a.file_url} />
            )}
          </li>
        ))}
...
function DeleteButton({ id, meetingId, filePath }: { id: string; meetingId: string; filePath: string }) {
  ...
  return (
    <button
      ...
      onClick={async () => {
        setPending(true);
        await deleteMeetingAttachment(id, meetingId, filePath);
        setPending(false);
      }}
