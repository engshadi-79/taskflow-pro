 ...
 * AiAssistantPanel, stacked above it) - shows the conversation list only;
 * opening a conversation or the "+" new-chat flow both hand off to the
 * full /dashboard/chat page rather than re-implementing the message
 * thread inside this small popup.
 */
export function ChatWidget() {
  ...
  // same "no filter, rely on RLS" shape as chat-app.tsx's own subscription
  useEffect(() => {
    const supabase = createClient();
  ...
  async function handleStartDirect(otherUserId: string) {
    ...
    setOpen(false);
    router.push(`/dashboard/chat?c=${result.id}`);
  }
            ...
            {filtered.map((c) => {
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    setOpen(false);
                    router.push(`/dashboard/chat?c=${c.conversation_id}`);
                  }}
