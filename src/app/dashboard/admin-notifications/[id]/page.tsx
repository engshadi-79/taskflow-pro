...
export default async function AdminNotificationDetailPage({
  ...
  const [{ data: notification }, { data: statsRaw }, { data: recipientsRaw }] = await Promise.all([
    supabase.from("admin_notifications").select("*").eq("id", id).single(),
  ...
  return (
    <div className="max-w-3xl space-y-4.5">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <div className="rounded-[14px] border border-border bg-surface p-3 text-center">
          <div className="text-[11px] text-faint">تاريخ الإرسال</div>
          <div className="mt-1 font-bold text-foreground">{notification.sent_at ? timeAgo(notification.sent_at) : "—"}</div>
        </div>
      </div>
