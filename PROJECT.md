...
| P11 | Organization Settings | ✅ — هوية (اسم/شعار/منطقة زمنية)، ساعات عمل + عطل رسمية، قيم افتراضية للمهام/SLA |
| P12–P25 | — | ⏳ لم تبدأ |
...
- **"ملاحظاتي" و"مهامي"** — لكل الأدوار لا الموظف فقط؛ ودجت "مهامي" كان
  يعرض عددًا فقط ("اليوم: 0") بلا أي قائمة تحته — عُدّل ليعرض المهام
  الفعلية (حتى 5، الأقدم أولًا) كل واحدة برابط للمهمة.
- **"آخر أنشطتي"** وُسّعت من 380px إلى 480px وأُزيل `truncate` عن نص كل
  نشاط، لأن الوصف كان يُقطع بثلاث نقاط دائمًا بصرف النظر عن طوله.
- **الإشعارات الإدارية (ميزة كاملة خارج ترقيم P01–P25)** — بثّ إشعارات من
  super_admin/department_manager للموظفين، بمرحلتين: (1) إرسال فوري + تتبع
  قراءة، يعيد استخدام جدول `notifications` الموجود بدل جدول مستلمين
  موازٍ؛ مدير القسم مقيّد بقسمه فقط (لا "الجميع" أبدًا) عبر دالة
  `send_admin_notification()` (لا سياسة INSERT مباشرة). (2) جدولة + تكرار
  + قوالب — المستلمون يُجمَّدون عند الإنشاء (`resolved_recipient_ids`) لا
  عند الإرسال، لأن التسليم المجدول/الدوري يعمل من pg_cron بلا `auth.uid()`؛
  التكرار جدول مستقل (`admin_notification_series`) لأن "قسمي بالكامل"
  يجب أن يُعاد حلّه كل مرة حسب من هو نشط فعليًا وقتها، لا وقت إنشاء القاعدة.

### التالي

عند قول المستخدم "اكمل": **P12**. لا تبدأ مرحلة تالية دون هذا الأمر
صريحًا (بروتوكول الخطة).
